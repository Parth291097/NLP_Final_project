...
def main():
    ...
    history = model.fit([train_input_ids, train_attention_masks], train_labels, batch_size=4, epochs=4)

    model.save_weights('./Model/xlnet_weights/xlnet_weights')

    # model.load_weights('./Model/xlnet_weights/xlnet_weights')

    preds = model.predict([test_input_ids, test_attention_masks])
    pred_labels = preds[0].argmax(axis=1)
    # np.savetxt("xlnet_predictions.csv", pred_labels, delimiter=",")
    ...
    subtaskB_pred_answers.to_csv('./Data/subtaskB_pred_xlnet_answers.csv', index=False)
