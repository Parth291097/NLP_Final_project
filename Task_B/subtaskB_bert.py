...
def main():
    ...
    train_input_ids, train_attention_masks = tokenize_sentences(train_sentences, bert_tokenizer)
    test_input_ids, test_attention_masks = tokenize_sentences(test_sentences, bert_tokenizer)
    ...
    # history = bert_model.fit([train_input_ids, train_attention_masks], train_labels, batch_size=32, epochs=4)

    # bert_model.save_weights('./Model/bert_weights')

    bert_model.load_weights('./Model/bert_weights')

    bert_preds = bert_model.predict([test_input_ids, test_attention_masks])
    bert_pred_labels = bert_preds[0].argmax(axis=1)
    # print(type(bert_pred_labels))
    # print(bert_pred_labels.shape)
    bert_prediction_answers = process_predictions(bert_pred_labels)
    subtaskB_pred_answers = pd.DataFrame(bert_prediction_answers)
    subtaskB_pred_answers.columns = ['id', 'ans']
    subtaskB_pred_answers.to_csv('./Data/subtaskB_pred_bert_answers.csv',index=False)
